...
public class StringUtil {
    ...
    public static String replace(Object target, Object... replacements) {
        return replace(target, 1, replacements);
    }
    ...
    public static String replaceAll(Object target, Object... replacements) {
        return replace(target, 0, replacements);
    }

    private static String replace(Object target, int limit, Object... replacements) {
        ...
        if (replacements.length % 2 == 0) {
            ...
            for (int i = 0; i < replacements.length; i += 2) {
                result = replace(result, replacements[i].toString(), replacements[i + 1].toString(), limit);
            }
            return result;
        }
        return null;
    ...
    private static String replace(String target, String regex, String replacement, int limit) {
        if (!target.isEmpty() && !regex.isEmpty()) {
            StringBuilder result = new StringBuilder();
            String[] targetCharacters = target.split("");
            String[] regexCharacters = regex.split("");
            ...
            for (int i = (limit > 0 ? 0 : targetCharacters.length - 1); (limit > 0 ? i < targetCharacters.length : i >= 0); i += (limit > 0 ? 1 : -1)) {
                ...
                if (Objects.equals(targetCharacters[0], regexCharacters[0])) {
                    boolean match = true;
